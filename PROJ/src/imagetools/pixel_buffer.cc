...
void PixelBuffer::SaveToFile(const std::string &filename) {
  imageio::Image* image = new imageio::Image(width(), height(), 4);
  for (int x = 0; x < image->Width(); x++) {
    for (int y = 0; y < image->Height(); y++) {
      ColorData copy = pixel(x, y);
      image->SetFloatValue(x, y, 0, copy.red());
      image->SetFloatValue(x, y, 1, copy.green());
      image->SetFloatValue(x, y, 2, copy.blue());
      image->SetFloatValue(x, y, 3, copy.alpha());
    }
  }
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  ...
  for (int x = 0; x < image->Width(); x++) {
    for (int y = 0; y < image->Height(); y++) {
      ColorData copy = ColorData(image->FloatValue(x, y, 0),
      image->FloatValue(x, y, 1), image->FloatValue(x, y, 2),
      image->FloatValue(x, y, 3));
      set_pixel(x, y, copy);
    }
  }
}
